...
enum{BOX,REGION,SINGLE,RANDOM, SINGLESPHERE};
enum{ATOM,MOLECULE};
...
void CreateAtoms::command(int narg, char **arg)
{
  ...
  if (strcmp(arg[1],"box") == 0) {
    ...
    iarg = 5;
  } else if (strcmp(arg[1],"single/sphere") == 0) {
    style = SINGLESPHERE;
    if (narg < 7) error->all(FLERR,"Illegal create_atoms command");
    xone[0] = utils::numeric(FLERR,arg[2],false,lmp);
    xone[1] = utils::numeric(FLERR,arg[3],false,lmp);
    xone[2] = utils::numeric(FLERR,arg[4],false,lmp);
    rad = utils::numeric(FLERR,arg[5],false,lmp);
    nsphere = utils::inumeric(FLERR,arg[6],false,lmp);
    iarg = 7;
  ...
  if (style == SINGLE) add_single();
    else if (style == SINGLESPHERE) add_singlesphere();
...
/* ----------------------------------------------------------------------
   add N atoms located on a sphere around the given center xone 
------------------------------------------------------------------------- */

void CreateAtoms::add_singlesphere()
{
  int d;
  double xcenter[3];
  for ( d=0; d<3; d++) xcenter[d] = xone[d];
  
  // calculate coordiantes around xone
  int count = 0;
  double a =  4*PI/nsphere;
  double s = sqrt(a);
  int Mtheta = DIV_ROUND_CLOSEST(PI,s);
  double dtheta = PI/Mtheta;
  double dphi = a/dtheta;
  for ( int m = 0;  m < Mtheta ; m++ ) {
    double theta = PI*(m+0.5)/Mtheta;
    int Mphi = DIV_ROUND_CLOSEST(2*PI*sin(theta),dphi);
    for ( int o = 0;  o < Mphi ; o++ ) {
      double phi = 2*PI*o/Mphi;
      xone[0] = rad * sin(theta) * cos (phi) + xcenter[0];
      xone[1] = rad * sin(theta) * sin (phi) + xcenter[1];
      xone[2] = rad * cos(theta) + xcenter[2];
      // create particle if in subbox
      if (xone[0] >= sublo[0] && xone[0] < subhi[0] &&
	xone[1] >= sublo[1] && xone[1] < subhi[1] &&
	xone[2] >= sublo[2] && xone[2] < subhi[2]) {
  ...
  if (count != nsphere)
    error->warning(FLERR,"Number of atoms created not equal to desired value. Change value if necessary.");
}
